...
function RegrasPage() {
    ...
    return (<SiteLayout>
      <PageHero eyebrow="Comunidade" title="Regras do servidor" description="Jogue limpo. As regras existem para manter a competição justa e o continente saudável."/>
      <section className="mx-auto max-w-3xl px-4 py-12 sm:px-6">
        {isPending ? <LoadingState label="Carregando regras oficiais..."/> : null}
        
        {isError ? (<ErrorState description="Não foi possível carregar as regras da API." onRetry={() => void refetch()}/>) : null}

        {!isPending && !isError && rules.length === 0 ? (<EmptyState title="Nenhuma regra definida" description="As regras oficiais serão publicadas em breve."/>) : null}

        <ol className="flex flex-col gap-6">
          {rules.map((rule) => (<li key={rule.id} className="surface-card p-6">
              <div className="mb-3 flex items-start justify-between gap-4">
                <h2 className="card-title text-ivory">{rule.title}</h2>
                <TagBadge tone={rule.severity === "critical" ? "danger" :
              ...
              <ul className="flex flex-col gap-2 text-sm leading-relaxed text-mist">
                {rule.details.map((detail, idx) => (<li key={idx} className="flex gap-2">
