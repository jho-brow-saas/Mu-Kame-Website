...
export function VipSection() {
    return (<MaterialSection material="iron">
      <SectionHeading eyebrow="VIP" title="Insígnias VIP" description="Assinaturas de 30 dias com experiência e drop ampliados. Os benefícios complementares serão divulgados antes do lançamento." className="mb-8"/>
      <VipPlansGrid />
      <div className="mt-8">
...
export function NewsSection() {
    ...
    return (<MaterialSection material="parchment">
      ...
      {isError ? (<ErrorState description="Não foi possível carregar os comunicados agora." onRetry={() => void refetch()}/>) : null}
      {!isPending && !isError && items.length === 0 ? (<EmptyState title="Nada publicado ainda" description="Os primeiros comunicados do MU Kame serão publicados em breve."/>) : null}
      {items.length > 0 ? (<ul className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => {
                const published = formatBrDate(item.publishedAt);
                return (<li key={item.id} className="plate plate-cut-soft flex flex-col gap-3 p-5 hover:border-gold/50">
                ...
                <Link to="/noticias/$slug" params={{ slug: String(item.id) }} className={cn(bracketClasses, "mt-auto")}>
...
export function DownloadsSection() {
    ...
    const items = data?.items ?? [];
    return (<MaterialSection material="launcher">
      <div className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-center">
        <SectionHeading eyebrow="Downloads" title="Prepare-se para entrar no continente." description="Cliente completo para Windows, patch de atualização e uma versão Android em desenvolvimento."/>
        <div className="plate plate-cut-slot overflow-hidden">
          ...
          <div className="flex flex-col gap-3 p-5">
            ...
            {isError ? (<ErrorState description="Não foi possível carregar os downloads agora." onRetry={() => void refetch()}/>) : null}
            {!isPending && !isError && items.length === 0 ? (<EmptyState title="Nenhum arquivo publicado" description="Os arquivos oficiais aparecerão aqui assim que forem liberados."/>) : null}
            {items.map((item) => item.available && isSafeExternalUrl(item.url) ? (<ActionAnchor key={item.id} href={item.url} target="_blank" rel="noopener noreferrer" variant={item.type === "client" ? "primary" : "secondary"}>
                  <Download className="size-4" aria-hidden="true"/>
                  Baixar {item.name}
                </ActionAnchor>) : (<p key={item.id} className="flex items-center gap-2 font-mono text-[0.7rem] uppercase tracking-[0.14em] text-ash">
...
export function CommunitySection() {
    ...
    return (<MaterialSection material="stone">
      <div className="grid gap-8 lg:grid-cols-[1fr_1fr] lg:items-center">
        <SectionHeading eyebrow="Comunidade e suporte" title="Precisa de ajuda?" description="Nossa equipe de suporte está disponível pelo WhatsApp oficial."/>
        <div className="plate plate-cut-slot overflow-hidden">
          ...
          <div className="flex flex-col gap-3 p-5">
            ...
            <div className="flex flex-col gap-3 sm:flex-row">
              {socials?.instagram.available !== false && (<ActionAnchor href={socials?.instagram.url ?? serverConfig.instagramUrl} target="_blank" rel="noopener noreferrer" variant="ghost" className="flex-1">
                  <Instagram className="size-4" aria-hidden="true"/>
                  Instagram {handle}
                </ActionAnchor>)}
              {socials?.tiktok.available !== false && (<ActionAnchor href={socials?.tiktok.url ?? serverConfig.tiktokUrl} target="_blank" rel="noopener noreferrer" variant="ghost" className="flex-1">
                  <Music2 className="size-4" aria-hidden="true"/>
                  TikTok {handle}
                </ActionAnchor>)}
              {socials?.discord.available && (<ActionAnchor href={socials.discord.url} target="_blank" rel="noopener noreferrer" variant="ghost" className="flex-1">
...
export function FaqSection() {
    ...
    return (<MaterialSection material="iron">
      <div className="mx-auto max-w-4xl">
        ...
        {isError ? (<ErrorState description="Não foi possível carregar o FAQ agora." onRetry={() => void refetch()}/>) : null}

        {!isPending && !isError && faqs.length === 0 ? (<EmptyState title="Nenhuma pergunta encontrada" description="As perguntas frequentes serão listadas em breve."/>) : null}

        <div className="flex flex-col gap-2">
          {faqs.map((item, index) => (<details key={item.id} className="plate plate-cut-soft group px-5 py-4">
...
export function FinalCta() {
    return (<section className="fortress-sheet relative isolate overflow-hidden edge-rule-top py-20">
